...
@section('content')
  ...
  <h1 class="w-max mx-auto text-2xl"><a href="/max">Путевой Лист</a> : <a href="{{ route('max.create') }}">Создать</a></h1>
      ...
      <div class="edit_form hidden">
        <div class="w-full h-screen flex justify-center items-center backdrop-blur-md fixed top-0 left-0">
          <div class="w-full sm:w-110 shadow-lg shadow-black bg-neutral-950/75">
            <form action="{{ route('max.update', $item->id) }}" method="POST" class="flex flex-col gap-1 px-3 py-10">
              @csrf
  ...
  <script>
    ...
    document.querySelectorAll('.delete_button').forEach((el) => {
      el.addEventListener('click', () => {
        ...
        document.querySelector('.delete_confirm').href =
          `?deleteOrder=${el.closest('.sticker').querySelector('.order_number').textContent}`
        // console.log(el.closest('.sticker').querySelector('.order_number').textContent);
      })
    })
